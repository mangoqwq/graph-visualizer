...
import { Graph, addEdge, Vertex } from "./graph";
import { TotalContext, VertexState } from "./graph-visualizer";
...
function VertexView({
	...
	setVertexStates: React.Dispatch<React.SetStateAction<Map<number, VertexState>>>;
}) {
	...
	const onStop: DraggableEventHandler = (
		...
		if (Date.now() - vertexStates.get(v.id)!.heldAt! < CLICK_TIME_THRESHOLD &&
				new Vector(data.deltaX, data.deltaY).norm() <= 10) {
			console.log(vertexStates.get(v.id)!.heldAt!, Date.now())
			newFrozen = !newFrozen;
		}
		const newVStates = new Map<number, VertexState>(vertexStates);
		newVStates.set(v.id, { ...vertexStates.get(v.id)!, heldAt: null, frozen: newFrozen });
		setVertexStates(newVStates);
	};
...
function stepPhysics(
	...
	for (const v of graph.vertices) {
		...
		if (vertexStates.get(v.id)!.heldAt !== null || vertexStates.get(v.id)!.frozen) {
...
export default function GraphViewer() {
	const { graph, setGraph, vertexStates, setVertexStates } = useContext(TotalContext);
				...
				return (
					<>
						...
						<line
							...
							x2={x2}
							y2={y2}
							stroke={graph.directed ? "blue" : "black"}
							strokeWidth="2"
						/>
					</>
				);
			...
			{graph.vertices.map((v) => {
				return (
					<VertexView
						...
						setVertexStates={setVertexStates}
					/>
				);
			})}
		</svg>
